...
from pages.settings import current_cfg, get_cfg
...
def _run_prediction_sim(entry, n=30000):
    return run_prediction_sim(entry, current_cfg(), n=n)


_predicted_ebitda = predicted_ebitda
...
def render_backtesting():
    ...
    with st.spinner("Running simulation..."):
        bt = backtest_summary(
            entry_assumptions, actual_results,
            {"exit_ev": act_exit_ev, "net_debt_at_exit": act_net_debt,
             "sponsor_equity_entry": act_eq_entry, "moic": act_moic, "irr": act_irr},
            current_cfg(), n=30000)
    pred_ebitda = bt["predicted_ebitda"]
    pred_irr_dist = bt["predicted_irr_distribution"]
    pred_equity_entry = bt["predicted_equity_entry"]
    pred_exit_equity = bt["predicted_exit_equity"]
    pred_moic = bt["predicted_moic"]
    pred_irr_mean = bt["predicted_irr_mean"]
    ...
    with tabs[3]:
        actual_ebitda_margin = bt["actual_ebitda_margin"]
        pred_ebitda_margin = bt["predicted_ebitda_margin"]
        ...
        with col_b:
            ...
            insight_rows = [
                ...
                ("IRR outcome",
                 ...
                 f"Landed at {bt['actual_percentile']:.0f}th percentile."),
                ("Model accuracy",
