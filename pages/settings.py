...
from core.config import DEFAULTS, is_valid_corr  # noqa: F401  (re-exported)
from core.config import build_corr_matrix as _build_corr_matrix
...
def current_cfg() -> dict:
    """Every setting as currently set in this session."""
    return {k: get_cfg(k) for k in DEFAULTS}


def build_corr_matrix() -> np.ndarray:
    """Correlation matrix from this session's settings."""
    return _build_corr_matrix(current_cfg())
